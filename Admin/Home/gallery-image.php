...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'upload') {
                    ...
                    if ($file_size <= 5000000) {
                        ...
                        if (in_array($imageFileType, ["jpg", "jpeg", "png", "gif"])) {
                            if (move_uploaded_file($file_tmp_path, $target_file)) {
                                ...
                                $stmt = $conn->prepare("INSERT INTO gallery (photo_name, photo_path) VALUES (?, ?)");
        ...
        $stmt = $conn->prepare("UPDATE gallery SET photo_name=? WHERE photo_id=?");
        ...
        $stmt = $conn->prepare("SELECT photo_path FROM gallery WHERE photo_id=?");
...
<body>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="background-color: #31343b; border: none;">
                <div class="modal-header" style="border-bottom: none; display: flex; justify-content: center;">
                    <h1 class="modal-title fs-5 text-danger" id="exampleModalLabel">Upload Image</h1>
                ...
                <div class="modal-body">
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <input type="text" class="form-control" name="image_name" placeholder="Name of the Image" required>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="background-color: #31343b; border: none;">
                <div class="modal-header" style="border-bottom: none; display: flex; justify-content: center;">
                    <h1 class="modal-title fs-5 text-danger" id="editModalLabel">Edit Image</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <button type="submit" class="btn btn-success">Update Image Name</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                            ...
                            <tbody>
                                <?php while ($row = $result->fetch_assoc()) : ?>
                                    <tr>
                                        ...
                                        <td><img src="<?= $row['photo_path'] ?>" alt="<?= $row['photo_name'] ?>" style="width: 100px; height: 150px;"></td>
                                        <td>
                                            <button class="btn btn-warning btn-sm edit-btn" data-id="<?= $row['photo_id'] ?>" data-name="<?= $row['photo_name'] ?>" data-bs-toggle="modal" data-bs-target="#editModal"><i class="fas fa-pencil-alt"></i> Edit </button>
                                            <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" method="post" style="display:inline;">
                                                <input type="hidden" name="action" value="delete">
                                                <input type="hidden" name="id" value="<?= $row['photo_id'] ?>">
                                                &nbsp;&nbsp;
                    ...
                    <?php else : ?>
                        <div class="alert alert-warning" role="alert">No gallery data available.</div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
